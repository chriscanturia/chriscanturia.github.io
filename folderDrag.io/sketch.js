// this uses library p5.play: http://molleindustria.github.io/p5.play/
...
function draw() {
  ...
  // check collision boxes: uncomment this if you want to see wire frames for debugging
...
function mouseDragged() {
  ...
  // check to see which folder is being trag
  ...
  // if folder is being dragged, update coordinates and add kick animation
...
function mouseReleased() {
  ...
  // change animation to still and drop folder when mouse released
  dragObjectIndex = null;
  folder1.changeAnimation("still");
  folder2.changeAnimation("still");
  folder3.changeAnimation("still");

  // If item is over trash, drop item in trash & update score counter
  for (i = 0; i < folders.length; i++) {
    if (folders[i] && folders[i].overlap(trash)) {
      folders[i].remove();
      //console.log(folders);
